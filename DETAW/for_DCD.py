...
def timeseries_combine(DETAWOUTPUT, Oldisl, Newisl, Numcrop, idates, ratefile):
        ...
        for iland in range(0, Oldisl):  # 168):
            if iterm != 4:
                ...
                for idd in range(0, idates):
                    DETAWISL168[iterm, iland, idd] = sum(
                        DETAWISLold[iterm, iland, 0:tempcrop, idd])
            else:
                DETAWISL168[4, iland, :] = DETAWISLold[0, iland, Numcrop-1, :] \
                    - DETAWISLold[2, iland, Numcrop-1, :]
                DETAWISL168[4, iland, :] = clip(
                    DETAWISL168[4, iland, :], 0.0, None)
    ...
    for iterm in range(0, 5):  # 5):
        for iland in range(0, Newisl):
            if iterm == 2:
                DETAWISLnew[iterm, iland, :] = DETAWISL168[iterm, iland, :] \
                    - DETAWISL168[5, iland, :]
                DETAWISLnew[iterm, iland, :] = clip(
                    DETAWISLnew[iterm, iland, :], 0.0, None)
            else:
                DETAWISLnew[iterm, iland, :] = clip(
                    DETAWISL168[iterm, iland, :], 0.0, None)
    return DETAWISLnew
...
def forNODCU(DETAWISL168, inputversion, model_start_year, endyear, ilands, outfilenames,daysofyear):
    ...
    for ifile in range(0, 5):
        ...
        for iland in range(1, ilands+1):
            if ifile == 3:
                strt = str(
                    "%3i" % iland)+"AT1 4 HISTORIC DEPLETION OF APPLIED WATER BY IRR. AND URBAN, AREA   "
                strt = strt + str(iland) + "\n"
                f4.writelines(strt)
                ...
                strt = str("%3i" % iland) + \
                    "AT1 4  TOTAL HISTORIC CU OF SEEPAGE  "
                strt = strt + str(iland) + "\n"
                f2.writelines(strt)
                strt = str("%3i" % iland)+"AT2            FROM DETAW STUDY \n"
                f2.writelines(strt)
                ...
                strt = str("%3i" % iland) + \
                    "AT1 4 TOTAL NET DEPLETION of WATERBODY, AREA   "
                strt = strt + str(iland) + "\n"
                f5.writelines(strt)
                strt = str("%3i" % iland)+"AT2            FROM DETAW STUDY \n"
                f5.writelines(strt)
                ...
                for iday in range(0, 366):
                    if (iday == 365 and daysofyear[iday, 0, iyr] != 0) or (iday < 365):
                        if ifile == 2:
                            ...
                            tempts[iyr, iday] = \
                                DETAWISL168[ifile, iland-1, ndays]
                        ndays += 1
            for iyr in range(0, tyr):
                ...
                for iday in range(0, 366):
                    if (iday == 365 and daysofyear[iday, 0, iyr] != 0) or (iday < 365):
                        if ifile == 4 and inputversion.strip() == "SCHISM":
                            # Set surface water evaporation as zero for SCHISM  11/28/2017
                            tempts[iyr][iday] = 0.0
