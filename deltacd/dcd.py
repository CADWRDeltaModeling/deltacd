...
def set_dcd_node_global_attributes(
    ...
    history = f"{created_at} - generated by DeltaCD version {__version__}, DCD module"
...
def island_to_nodes(ds_dcd, input_data):
    ...
    # Depletion over open water
    da_depletion_openwater_nodes = ds_dcd.depletion_from_waterbody.dot(
        ds_dist_rates.divrate
    )
    da_depletion_openwater_nodes = da_depletion_openwater_nodes.assign_attrs(
        units="cfs", description="depletion over open water at model nodes"
    )
    ...
    ds_dcd_nodes["depletion_from_waterbody"] = da_depletion_openwater_nodes
...
def calculate_depletion(model_params: dict, input_data: dict) -> xr.Dataset:
    ...
    da_depletion_waterbody *= taf2cfs
    ds_dcd["depletion_from_waterbody"] = da_depletion_waterbody
    ds_dcd["depletion_from_waterbody"].attrs = {
        "units": "cfs",
        "description": "depletion from open water at subarea",
    }
